...
const SHORT_LABELS: Record<NodeId, string> = {
  1: "Node 1",
  2: "Node 2",
  3: "Node 3",
  4: "Node 4",
  ...
  if (goldPity != null) {
    return (
      <td className="px-1 py-1 text-center">
        <span
          className={`inline-flex items-center gap-1 rounded px-2 py-0.5 font-semibold tabular-nums ${
            LUCK_BG[goldPityColor(goldPity)]
          }`}
        >
...
// "Node 1 *, Node 2 *") instead of the longer "-> Gold" text. Other milestone
...
function MilestoneRow({
  ...
  return (
    <tr className={isLatest ? "bg-surface-hover/40" : ""}>
      <td className="sticky left-0 z-10 bg-surface px-2 py-1.5 text-center text-muted tabular-nums">
        {m.index}
      </td>
    ...
    <section className="flex flex-col gap-2">
        ...
        <table className="w-full min-w-[640px] border-collapse text-sm">
          <thead>
            <tr className="border-b border-border bg-surface text-left text-xs uppercase tracking-wide text-muted">
              <th className="sticky left-0 z-10 bg-surface px-2 py-2 text-center">
                #
              </th>
          ...
          <tbody className="divide-y divide-border">
            {/* Start baseline row */}
            <tr>
              <td className="sticky left-0 z-10 bg-surface px-2 py-1.5 text-center text-muted">
                0
              </td>
            ...
            {milestones.map((m, i) => (
              <MilestoneRow
                key={m.input.id}
                m={m}
                isLatest={i === milestones.length - 1}
      ...
      {milestones.length === 0 && (
        <p className="text-sm text-muted">
          No milestones yet. Use "Add Rolls" each time a node turns gold so
          nothing is missed.
        </p>
      )}
    </section>
