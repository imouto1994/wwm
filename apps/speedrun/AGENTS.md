...
- A tool for the guild's speedrun event: the host randomizes a DPS weapon, a 2nd (different) weapon, and 8 unique mystic skills, then shares the exact result via a URL.
...
- `src/types/randomizer.ts` - domain types. `WeaponEntry.isDps` decides eligibility for the 1st randomized weapon; `SkillEntry` has no category (one flat pool). `LoadoutResult` is just 3 ids/id-lists - the only thing ever persisted (into the URL).
...
- `src/data/data.test.ts` - guards the pools: unique ids, non-empty fields, at least 1 DPS weapon, at least 8 skills. Keep this green - it is the safety net for careless data edits.
- `src/lib/randomize.ts` - **pure, framework-free** `randomizeLoadout(weapons, skills, rng?)`. Weapon 1 from the DPS-filtered pool; weapon 2 from the full pool minus weapon 1's id (must differ); 8 unique skills via a partial Fisher-Yates shuffle. Takes an injectable `rng` for deterministic tests. Throws a descriptive error on too-small pools - callers must catch and show it, not let it crash the UI.
...
- `src/components/` - `EntryCard` (generic weapon/skill card, fixed 1:1 image frame + optional badge), `ImageWithFallback` (falls back to an initials box on a 404/decode failure, regardless of image format), `LoadoutResult` (2 weapon cards + an 8-up skill grid, skills sorted back to their pool order for a stable display), `RandomizeButton` and `ShareLink` (both host-mode only).

## Invariants that are easy to get wrong

- **Weapon 2 must differ from weapon 1** even though the DPS pool is a subset of the full pool - `randomizeLoadout` excludes weapon 1's id from weapon 2's candidate pool. Don't "simplify" this back to an unfiltered pick.
